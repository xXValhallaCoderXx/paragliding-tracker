...
import { Button, LinkButton, PulseDot } from '@/components/flight-ui';
import type { FlightSummary } from '@/recorder/types';
import { flightLocalDate, formatClockTime } from '@/ui/flight-format';
        ...
        {interrupted ? (
          <>
            ...
            <Button
              ...
              onPress={onFinalize}
              disabled={disabled}
              style={[styles.action, styles.actionAttention]}
            />
          </>
        ) : (
          <Button label="Open recorder" variant="dark" onPress={onOpenRecorder} style={styles.action} />
        )}
      </View>
      {interrupted ? (
        <LinkButton label="Look at the recorder first ›" onPress={onOpenRecorder} style={styles.link} />
      ) : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  action: { flex: 1 },
  actionAttention: { borderColor: paper.attentionBorder },
  link: { alignSelf: 'center', marginTop: 4, marginBottom: -6 },
});
